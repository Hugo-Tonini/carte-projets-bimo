...
(() => {
  ...
  // Bouton Réinitialiser : on le met en bleu quand un filtre “antenne” est actif
  let clearBtnDefaultStyle = null;
  function setClearButtonBlue(isBlue) {
    if (!elClear) return;

    if (!clearBtnDefaultStyle) {
      const cs = getComputedStyle(elClear);
      clearBtnDefaultStyle = {
    ...
    if (isBlue) {
      elClear.style.backgroundColor = "#1e90ff";
      elClear.style.borderColor = "#1e90ff";
      elClear.style.color = "#fff";
    } else {
      elClear.style.backgroundColor = clearBtnDefaultStyle.backgroundColor;
      elClear.style.borderColor = clearBtnDefaultStyle.borderColor;
      elClear.style.color = clearBtnDefaultStyle.color;
    }
  }
    ...
    for (const o of OFFICES) {
      ...
      marker.on("click", (e) => {
        ...
        if (o.type_lieu === "antenne" && o.antenne) {
          ...
          updateDeptStyle();
          // Indique visuellement que la vue est “filtrée antenne” et qu'on peut réinitialiser
          setClearButtonBlue(true);
        } else {
          clearAntennaFocus();
        }
