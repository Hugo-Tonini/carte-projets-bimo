...
marker.on("click", (e) => {
  ...
  function normalizeLightboxItems(items, fallbackSrc, fallbackAlt) {
    const out = [];
    const seen = new Set();
    const pushItem = (src, alt) => {
      ...
      seen.add(cleanSrc);
      out.push({ src: cleanSrc, alt: String(alt || "Photo") });
    };

    if (Array.isArray(items)) {
      for (const item of items) {
        if (typeof item === "string") {
          pushItem(item, fallbackAlt);
    ...
    if (!out.length && fallbackSrc) pushItem(fallbackSrc, fallbackAlt);
    return out;
  }
  ...
  function renderLightboxImage() {
    ...
    const prevBtn = lightboxEl.querySelector('[data-nav="prev"]');
    const nextBtn = lightboxEl.querySelector('[data-nav="next"]');
    if (!img || !current) return;

    img.src = current.src;
    img.alt = current.alt || "Photo";
  ...
  function openLightbox(src, alt, items, startIndex = 0) {
    const normalized = normalizeLightboxItems(items, src, alt);
    ...
    const foundIndex = requestedSrc ? normalized.findIndex((item) => item.src === requestedSrc) : -1;
    lightboxIndex = foundIndex >= 0 ? foundIndex : Math.max(0, Math.min(Number(startIndex) || 0, normalized.length - 1));

    lightboxEl = document.createElement("div");
    lightboxEl.className = "lightbox";
    lightboxEl.innerHTML = `
      <div class="lightboxBackdrop" data-close="1"></div>
      <div class="lightboxContent" role="dialog" aria-modal="true" aria-label="Album photo">
        <button class="lightboxNav lightboxNavPrev" type="button" aria-label="Photo précédente" data-nav="prev">‹</button>
        <div class="lightboxFigureWrap">
        ...
        <button class="lightboxNav lightboxNavNext" type="button" aria-label="Photo suivante" data-nav="next">›</button>
      </div>
    `;
    ...
    lightboxEl.addEventListener("click", (e) => {
      ...
      if (nav === "prev") moveLightbox(-1);
      if (nav === "next") moveLightbox(1);
    });
  ...
  function showPanel(p) {
    ...
    // clic sur photo principale ou miniatures -> lightbox avec navigation dans l'album
    const photoButtons = elPanel?.querySelectorAll(".projPhotoBtn") || [];
    const albumItems = photos.map((photoSrc, i) => ({ src: String(photoSrc), alt: `Photo ${i + 1}` }));
    photoButtons.forEach((btn) => {
      btn.addEventListener("click", () => {
        const src = btn.getAttribute("data-src") || "";
        const index = albumItems.findIndex((item) => item.src === src);
        if (src) openLightbox(src, btn.getAttribute("aria-label") || "Photo", albumItems, index);
      });
    });
    const btn = document.getElementById("panelClose");
    if (btn) btn.addEventListener("click", () => closePanel({ resetView: true }), { once: true });
  ...
  function renderHeroPhoto(src) {
    ...
    return `
      <div class="projHero">
        <button class="projPhotoBtn projHeroBtn" type="button" data-src="${escapeHtml(safe)}" aria-label="${escapeHtml(alt)}">
