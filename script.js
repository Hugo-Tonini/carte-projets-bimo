...
marker.on("click", (e) => {
  ...
  function renderPhotosHtml(photos, title) {
    ...
    return `
      <div class="projPhotos" style="margin-top:16px;">
        <div class="projPhotosTitle" style="font-weight:800;font-size:16px;">Photos</div>
  ...
  function showPanel(p) {
    ...
    if (resumeTxt) {
      ...
      html += `<div class="panelSummary" style="margin:14px 0 12px;padding:10px 12px;border:1px solid rgba(0,0,0,0.08);border-radius:12px;background:rgba(255,255,255,0.75);">
                <div style="font-weight:800;font-size:16px;margin-bottom:6px;">Résumé</div>
                <div style="line-height:1.25;">${safe}</div>
              </div>`;
    }

    html += `<div class="panelSubTitle" style="margin-top:14px;font-weight:800;font-size:16px;letter-spacing:.02em;">Infos générales</div>`;
    html += buildKv(fieldsMain);

    // Sous-titre "Contacts" + infos
    const hasContacts = fieldsContacts.some(([, v]) => v !== undefined && v !== null && String(v).trim() !== "");
    if (hasContacts) {
      html += `<div class="panelSubTitle" style="margin-top:16px;font-weight:800;font-size:16px;letter-spacing:.02em;">Contacts</div>`;
    ...
    if (hasEnergy) {
      html += `<div class="panelSubTitle" style="margin-top:16px;font-weight:800;font-size:16px;letter-spacing:.02em;">Infos énergétiques</div>`;
  ...
  function buildKv(fields) {
    ...
    for (const [label, value] of fields) {
      ...
      // - valeurs centrées
      html += `
        <div class="kvRow" style="display:grid;grid-template-columns: 0.8fr 1fr;gap:12px;align-items:center;">
          <div class="kvKey" style="color:#000;font-weight:800;font-size:15px;line-height:1.25;">${escapeHtml(label)} :</div>
