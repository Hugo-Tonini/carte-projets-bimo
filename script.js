...
(() => {
  ...
  function buildDeptNameMap(geo) {
    ...
    for (const f of features) {
      ...
      if (code && key) mapNameToCode.set(key, code);
    }
  ...
  function onEachDept(feature, layer) {
    ...
    // Tooltip dynamique : nom + code + nb de projets filtrés
    const n = () => (lastCounts[code] || 0);
    layer.bindTooltip(() => {
      const tail = code ? ` (${code})` : "";
      return `${escapeHtml(name)}${tail} — ${n()} projet(s)`;
    }, { sticky: true });
  }
