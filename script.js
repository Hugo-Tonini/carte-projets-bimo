...
clusters.on("clustermouseover", (a) => {
  ...
  const max = 25;
  let html = names.slice(0, max).map(n => escapeHtml(n)).join("<br>");
  if (names.length > max) html += `<br>+${names.length - max} autres`;
  if (!html) html = `${kids.length} projets`;
  if (!cl.getTooltip()) {
    cl.bindTooltip(html, { className: "projTooltip", direction: "top", offset: [0, -10], opacity: 0.95, sticky: true });
  } else {
