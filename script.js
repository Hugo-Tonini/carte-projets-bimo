...
marker.on("click", (e) => {
  ...
  function showPanel(p) {
    ...
    if (resumeTxt) {
      ...
      html += `<div class="panelSummary" style="margin:10px 0 8px;padding:10px 12px;border:1px solid rgba(0,0,0,0.08);border-radius:12px;background:rgba(255,255,255,0.75);">
                <div style="font-weight:800;margin-bottom:4px;">Résumé</div>
...
html += buildKv(fieldsMain);
    ...
    if (hasContacts) {
      html += `<div class="panelSubTitle" style="margin-top:10px;font-weight:800;letter-spacing:.02em;">Contacts</div>`;
    ...
    if (hasEnergy) {
      html += `<div class="panelSubTitle" style="margin-top:10px;font-weight:800;letter-spacing:.02em;">Infos Énergétiques</div>`;
  ...
  function buildKv(fields) {
    ...
    for (const [label, value] of fields) {
      ...
      // - valeurs centrées
      html += `
        <div class="kvRow" style="display:grid;grid-template-columns: 1fr 1fr;gap:12px;align-items:center;">
          <div class="kvKey" style="color:#000;font-weight:700;font-size:14px;line-height:1.25;">${escapeHtml(label)} :</div>
          <div class="kvVal" style="color:#000;text-align:center;justify-self:center;line-height:1.25;">${escapeHtml(s)}</div>
        </div>`;
    }
    html += `</div>`;
    return html;
