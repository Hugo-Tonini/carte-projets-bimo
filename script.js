  ...
  function enrichProjectsWithDepartments(projects) {
    for (const p of projects) {
      ...
      if (/^(\d{2}|\d{3}|2A|2B)$/.test(existingCode)) {
        ...
        p.__deptName = String(p.__deptName ?? p["Nom département"] ?? deptCodeToName[existingCode] ?? "").trim();
        continue;
      }
      ...
      const inferred = inferDeptFromLatLon(ll[0], ll[1]);
...
marker.on("click", (e) => {
  ...
  function updateDeptSelectedStat() {
    ...
    if (filterByDeptEnabled && selectedDeptCode) {
      const name = String(deptCodeToName[selectedDeptCode] ?? "").trim();
      const label = name ? `${name} (${selectedDeptCode})` : selectedDeptCode;
      elStatDept.textContent = `— département : ${label}`;
