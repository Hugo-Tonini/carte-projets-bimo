...
clusters.on("clustermouseout", (a) => {
  ...
  function showOfficePanel(o) {
    ...
    html += `</div>`;
    html += buildKv(fields);
    html += renderOfficeProjectsSection(o);

    openPanel(html);

    const list = elPanel?.querySelector('.officeProjectsList');
    if (list) {
      list.addEventListener('click', (ev) => {
        const row = ev.target?.closest?.('.officeProjectRow');
        if (!row) return;
        const pid = row.getAttribute('data-project-id');
        if (pid) openProjectFromAntennaPanel(pid);
      });
    }
...
function amountNumber(v) {
  ...
  function projectsForAntenna(antennaName) {
    ...
    return allProjects
      ...
      .slice()
      .sort((x, y) => {
        const nx = String(x["Nom de projet"] ?? x.nom ?? "").trim();
        const ny = String(y["Nom de projet"] ?? y.nom ?? "").trim();
        return nx.localeCompare(ny, 'fr');
      });
  }

  function renderOfficeProjectsSection(o) {
    if (!o || o.type_lieu !== "antenne" || !o.antenne) return "";

    const projects = projectsForAntenna(o.antenne);
    if (!projects.length) return `
      <div class="officeProjects" style="margin-top:20px;">
        <div class="officeProjectsTitle">Projets</div>
        <div class="officeProjectsEmpty">Aucun projet pour cette antenne.</div>
      </div>`;

    const rows = projects.map((p) => {
      const pid = projectId(p);
      if (!pid) return '';
      const nom = String(p["Nom de projet"] ?? p.nom ?? "").trim() || '(sans nom)';
      const typ = String(p["Type de projet"] ?? p.type ?? "").trim() || '—';
      const ville = projectCity(p) || '—';
      return `
        <button class="officeProjectRow" type="button" data-project-id="${escapeAttr(pid)}">
          <div class="officeProjectName">${escapeHtml(nom)}</div>
          <div class="officeProjectMeta">
    ...
    return `
      <div class="officeProjects" style="margin-top:20px;">
        <div class="officeProjectsTitle">Projets</div>
        <div class="officeProjectsList">${rows}</div>
      </div>`;
  }

  function openProjectFromAntennaPanel(pid) {
