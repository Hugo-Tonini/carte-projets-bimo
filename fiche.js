(() => {
  ...
  function renderIdentite(p) {
    ...
    // Si tu ajoutes plus tard des champs du type "Dates clés" ou "Avis MMA conforme", ils apparaitront ici
    const knownDates = [
  ...
  function renderActus(p) {
    ...
    // Format flexible :
    // - si p["Vigilance"] est une liste (array), on la prend
    // - sinon on essaye un texte multi-lignes
  ...
  function fillTop(p) {
    el("title").textContent = p["Nom de projet"] || "Projet";
    el("maj").textContent = p["Maj"] || p["Màj"] || p["Mise à jour"] || new Date().toLocaleDateString("fr-FR");
    el("adresse").textContent = p["Adresse"] || "";
    el("equipe").textContent = p["Équipe projet"] || p["Equipe projet"] || "";
    el("processus").textContent = p["Type de projet"] || "";
    el("resume").textContent = p["Résumé"] || "";
  }

  async function load() {
    if (!code) {
      el("title").textContent = "Code projet manquant";
      el("resume").textContent = "Ouvrez la fiche depuis un projet (bouton “Fiche A4” dans le panneau latéral).";
      renderTimeline(-1);
      return;
    }
    ...
    if (!p) {
      el("title").textContent = `Projet introuvable (code ${code})`;
      el("resume").textContent = "Vérifiez le Code projet dans export_projets_web.json.";
      renderTimeline(-1);
      return;
    }
    ...
    el("identite").innerHTML = renderIdentite(p);
    el("actus").innerHTML = renderActus(p);
    el("contacts").innerHTML = renderContacts(p);
  ...
  load().catch((e) => {
    el("title").textContent = "Erreur";
    el("resume").textContent = String(e?.message || e);
    renderTimeline(-1);
  });
})();
